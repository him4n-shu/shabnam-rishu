...
$(function(){
	...
	// Virtual Gift Box Functionality
	$('.gift-box').on('click', function() {
		var giftType = $(this).data('gift');
		var modalContent = '';
		
		// Set content based on gift type
		if (giftType === 'love-note') {
			modalContent = `
				<div class="gift-content love-note">
					<h3>A Love Note Just For You</h3>
					<p>My Love,</p>
					<p>Happy Birthday to the most beautiful soul I know. You make my world brighter, my days happier, and my heart fuller. Every moment with you feels magical, and I'm grateful for your love, laughter, and presence in my life. You are my best friend, my safe place, and my greatest blessing.</p>
					<p class="signature">Forever yours. 💖</p>
				</div>
			`;
		} else if (giftType === 'video-message') {
			modalContent = `
			...
			modalContent = `
				<div class="gift-content special-promise">
					<h3>My Promise To You</h3>
					<p>I promise to always love you, support you, and cheer for you in everything you do.</p>
					<p>I promise to stay by your side in good times, bad times, and even the crazy times.</p>
					<p>I promise to share my food (well, most of the time 😜).</p>
					<p>And yes… I promise not to fight with you over silly things, because your smile means way more than winning any argument. 💖</p>
					<p class="signature">With all my love,<br>❤️</p>
				</div>
			`;
		}
		
		// Show modal with content
		$('#giftContentContainer').html(modalContent);
		// Toggle a smaller modal for video message only
		if (giftType === 'video-message') {
			$('#giftModal').addClass('video-modal');
		} else {
			$('#giftModal').removeClass('video-modal');
		}
		$('#giftModal').fadeIn();

		// If playing the background music, pause it when opening the video message
		if (giftType === 'video-message') {
	...
	$('.gift-close').on('click', function() {
		...
		$('#giftContentContainer').find('video').each(function(){
			this.pause();
			this.currentTime = 0;
		});
		$('#giftModal').fadeOut();
	...
	$('#unlockSecret').on('click', function() {
		var secretCode = $('#secretCode').val().trim();
		// The date code for unlocking the message
		var correctCode = "16022021"; 
		
		if (secretCode === correctCode) {
			$('#secretMessageContent').slideDown(500, function(){
			...
			$('.puzzle-clue').addClass('success');
		} else {
			$('#secretCode').addClass('shake-animation');
			setTimeout(function() {
				$('#secretCode').removeClass('shake-animation');
			}, 500);
			// Show toast instead of alert
			var $toast = $('#toastMessage');
			if ($toast.length) {
				$toast.text('That\'s not quite right. Try again!');
